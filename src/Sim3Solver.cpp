...
void Sim3Solver::ComputeSim3(cv::Mat &P1, cv::Mat &P2)
{
    ...
    // (cxn)opencv中的这个函数要求输入矩阵必须是对称矩阵！
    cv::eigen(N,eval,evec); //evec[0] is the quaternion of the desired rotation

    // N矩阵最大特征值（第一个特征值）对应特征向量就是要求的四元数死（q0 q1 q2 q3）
    // 将(q1 q2 q3)放入vec行向量，vec就是四元数旋转轴乘以sin(ang/2)
    // (cxn) q=[ cos(t), nx*sin(t/2), ny*sin(t/2), nz*sint(t/2) ]
    // 另外特征向量是单位向量 [nx*sin(t/2), ny*sin(t/2), nz*sint(t/2)], 其范数为sin(t/2)
    // ang=t/2
    // 旋转向量=2*ang* [nx*sin(t/2), ny*sin(t/2), nz*sint(t/2)] / sin(t/2)
    ...
    if(!mbFixScale)
    {
        ...
        // (cxn)这里用的是论文p632: D.Finding the Scale 中的公式
