...
int ORBmatcher::SearchByProjection(KeyFrame* pKF, cv::Mat Scw, const vector<MapPoint*> &vpPoints, vector<MapPoint*> &vpMatched, int th)
{
    ...
    // Decompose Scw
    // Scwd的形式为[sR, st]
    // (cxn)?这里的相似变换矩阵分解为啥是这样的，不太明白
    // 20.3.18 大概明白了，其实就是换算成匹配帧的尺度，
    // 在上一步Optimizer::OptimizeSim3优化中，我们求出了匹配帧到当前帧的相似变换 Scm= [scm*Rcm,tcm;0 1]
    // 然后将以 匹配帧的尺度为准，世界坐标系到匹配帧的变换为 Smw=[Rmw,tmw;0 1] (smw为1)
    // 世界坐标系到当前帧的变换为 Scw=Scm*Smw=[scw*Rcw,tcw;0 1] (scw=scm)
    // 这里为了让当前帧跟 匹配帧及链接帧中的地图点匹配，得把位移换算成匹配帧的尺度，即 位移向量/=scw
    // 除scw是很好理解的，比如 原坐标系中 点坐标为(1,1,1)， 经过相似变换 [sE,0;0 1]，得到点坐标在现坐标系下 坐标(s,s,s)
    // （原来尺度下，一单位代表 10cm；现在尺度下，一单位代表 10/s cm）
    // 为了换成原来的尺度，(s,s,s)/s=(1,1,1)
...
int ORBmatcher::SearchForInitialization(Frame &F1, Frame &F2, vector<cv::Point2f> &vbPrevMatched, vector<int> &vnMatches12, int windowSize)
{
    ...
    for(size_t i1=0, iend1=F1.mvKeysUn.size(); i1<iend1; i1++)
    {
        ...
        // (cxn)找到F2中在0尺度下的特征点集，在F1待匹配的特征点周围2windowSize×2windowSize周围搜索
        ...
        for(vector<size_t>::iterator vit=vIndices2.begin(); vit!=vIndices2.end(); vit++)
        {
            ...
            {
                bestDist2=bestDist;
                bestDist=dist;//(cxn)找到范明距离最小值和第二最小值，以及最小值对应的特征点（F2）
                bestIdx2=i2;
            }
            else if(dist<bestDist2)
            {
        ...
        if(bestDist<=TH_LOW)
        {
            ...
            {
                //(cxn)F1某个特征点可能与多盖F2中特征点范明距离比较近，后来的吃香 
...
int ORBmatcher::Fuse(KeyFrame *pKF, cv::Mat Scw, const vector<MapPoint *> &vpPoints, float th, vector<MapPoint *> &vpReplacePoint)
{
    ...
    for(int iMP=0; iMP<nPoints; iMP++)
    {
        ...
        if(bestDist<=TH_LOW)
        {
            ...
            // (cxn)我咋觉得是因为 m_pMap->m_MutexMapUpdate 这个互斥量，所以才不直接调用 Re_place()
            if(pMPinKF)
            {
                if(!pMPinKF->isBad())
            ...
            if(!LastFrame.mvbOutlier[i])
            {
                ...
                for(vector<size_t>::const_iterator vit=vIndices2.begin(), vend=vIndices2.end(); vit!=vend; vit++)
                {
                    ...
                    if(CurrentFrame.mvpMapPoints[i2])
                        ...
                        //(cxn)非单目的条件下，会有一些 观测量==0 的地图点临时加入，
                        // 匹配时，新帧中 A特征点会在某次匹配时 匹配到了 上帧中（obs==0）的地图点，我们认为不稳定
                        // 在后续的匹配中，可能会更改对应的地图点，除非匹配到了 上帧中（obs==0）的地图点
                        // 不过这样不就变成了后来居上了吗，而且特征点方向差中没有相应处理
                            continue;
...
// 取出直方图中值最大的三个index
// (cxn)若第二、第三大值比第一大值小得多，就只返回最大的那个index
