...
void Tracking::Track()
{
    ...
    else// 步骤2：跟踪
    {
        ...
        if(bOK)
        {
            ...
            //(cxn)跟踪成功之后，会把当前帧的位姿发给可视化线程
            // 同时，可视化程序会绘制所有地图点（不是bad的地图点，同时刨除参考地图点）为红色，
            // 参考地图点标为黑色，参考地图点来自与当前帧有共视点的关键帧集K1，与k1有共视链接的关键帧集k2中的地图点
            // 还会绘制所有关键帧，并画出每个关键帧与其共视链接最强的100个关键帧的链接
            // 每个关键帧的父帧与回环帧的也会绘制（20.3.13 还不清楚）
...
void Tracking::CreateInitialMapMonocular()
{
    ...
    // 评估关键帧场景深度，q=2表示中值
    // (cxn)上面注释中“云中值深度（欧式距离，不是指z）归一化到1”好像不对，代码中就是将z归一化到1的
    float medianDepth = pKFini->ComputeSceneMedianDepth(2);
    float invMedianDepth = 1.0f/medianDepth;

    if(medianDepth<0 || pKFcur->TrackedMapPoints(1)<100)
    {
        //(cxn)第二个条件是如果当前帧中的地图点中 至少被观测到一次的点 的总数少于100
...
void Tracking::UpdateLastFrame()
{
    ...
    // (cxn)跟踪过程中需要将 lastFrame 的MapPoints投影到当前帧，再以这个投影点为中心搜索匹配点。
    // 增加lastFrame的Mappoints 能增加两帧的匹配点，对当前帧位姿优化有好处
    ...
    for(size_t j=0; j<vDepthIdx.size();j++)
    {
        ...
        if(vDepthIdx[j].first>mThDepth && nPoints>100)
            break;
        //(cxn)也就是说若有深度的点超过100个，至少就要100个点（不管深度是不是超过了基线×35）
    }
}
...
bool Tracking::TrackWithMotionModel()
{
    ...
    {
        //(cxn)与地图点匹配点少于10个
        // 地图点匹配点 与 前一普通帧的匹配点，在单目情况下是一样的，非单目情况下则不同
        mbVO = nmatchesMap<10;
        //(cxn)与前一普通帧的匹配点超过20个
        return nmatches>20;
    }
...
bool Tracking::NeedNewKeyFrame()
{
    ...
    if(nKFs<2)
        ...
        //(cxn)估计是因为这时只有初始化的一帧，估计是此时地图点数比较多，所以才设了这么低的一个比例
    ...
    if(mnMatchesInliers>300)
        thMapRatio = 0.20f;//(cxn)此时地图点数足够多，严格一些
    ...
    if((c1a||c1b||c1c)&&c2)
    {
        ...
        {
            //(cxn)若local部分不空闲，就打断local
            // 若不是单目，若local部分中未处理的关键帧少于3个，就插；单目的话就不插了
        ...
        if(!vDepthIdx.empty())
        {
            ...
            for(size_t j=0; j<vDepthIdx.size();j++)
            {
                ...
                if(bCreateNew)
                {
                    ...
                    // (cxn) 这些之前没有的地图点 与 关键帧 提前建立了联系（obs==2）
                    // 在LocalMapping::Process_NewKeyFrame()，会发现这个提前增加的联系，并认为这些地图点并不稳定
                    // 因此加入到 mlp_RecentAddedMapPoints，等待甄别
 ...
 * 
 * (cxn)遍历当前帧的MapPoints，将观测到这些MapPoints的关键帧取出来，放入mvp_LocalKeyFrames（放入之前要清空）,
 * 且选出里面共视程度最高的那个关键帧作为 当前帧及追踪的参考帧，
 * 另外对mvp_LocalKeyFrames中分别找到其父帧、子帧们、共视程度最高的10关键帧，也加入mvp_LocalKeyFrames
 */
void Tracking::UpdateLocalKeyFrames()
{
    ...
    // (cxn)找到能关键帧中与当前帧有共视点的关键帧子集 及其共视点的个数
    for(int i=0; i<mCurrentFrame.N; i++)
    {
        if(mCurrentFrame.mvpMapPoints[i])
        {
