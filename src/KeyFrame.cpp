 ...
 * @brief 更新图的连接
 * 
 * 1. 首先获得该关键帧的所有MapPoint点，统计观测到这些3d点的每个关键帧与该关键帧之间的共视程度
...
void KeyFrame::UpdateConnections()
{
    ...
    int th = 15;//(cxn)共视数量大于这个阈值，就对这两个关键帧建立链接；如果没有超过阈值的权重，则对权重最大的关键帧建立连接
    ...
    //===============3==================================
    {
        ...
        // 更新生成树的连接
        // (cxn)某一关键帧刚插入时，可能与前面多个关键帧有共视链接，会将其与共视点最多的那个关键帧作为父帧，同时建立双向链接
...
void KeyFrame::SetErase()
{
    ...
    // (cxn)这里注释错了，这段代码没错的，local_Mapping中,若想删去这个帧，但因为 mb_NotErase==true 没删成，
    // 就将mb_ToBeErased置成true，以便 mb_NotErase从true变成false时，再删
    if(mbToBeErased)
    {
        SetBadFlag();
    }
