    ...
    for(size_t i=0, iend=vpCandidateKFs.size(); i<iend; i++)
    {
        ...
        for(size_t iG=0, iendG=mvConsistentGroups.size(); iG<iendG; iG++)
        {
            ...
            if(bConsistent)
            {
                ...
                //(cxn)上面的注释就应该是i，  
                // 不该是 vector<bool> vbConsistentGroup(mvConsistentGroups.size(),false);
                // 而是  vector<bool>  vbConsistentGroup(vpCandidateKFs.size(),false);
                // 下面的 vbConsistentGroup[iG]=true; 也应该是 vbConsistentGroup[i]=true;
                {
...
void LoopClosing::CorrectLoop()
{
    ...
    // (cxn)等待localmap完了之后，可能当前帧会得到一些新的地图点，需要重新链接下（LocalMapping::SearchInNeighbors()）
    // localmap中的当前帧与loopclosing中当前帧可能不是同一帧
    ...
    // (cxn)感觉这里的注释是错的，在上面的优化中，最开始建立边的就是此次检测出来的回环帧（用的是经过纠偏的定点与边）
    // 后面有一步是遍历所有关键帧：建立父子帧的边；建立回环帧的边；建立高共视帧的边（用的是未经过纠偏的定点与边）
    // 也就是说关键帧的回环帧是在优化之后加入的
